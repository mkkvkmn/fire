...
def add_transaction_row_id(df: pd.DataFrame) -> pd.DataFrame:
    if "transaction_row_id" not in df.columns:
        if "transaction_id" in df.columns and "owner" in df.columns:
...
def log_categorization_and_id_changes(
    ...
    try:
        ...
        df_new = add_transaction_row_id(df_new)
        ...
        if not df_changes.empty:
            ...
            log_id_changes(df_changes, output_dir, timestamp)
