...
def load_accepted_duplicates(accepted_file_path: str) -> pd.DataFrame:
    ...
    if os.path.exists(accepted_file_path):
        return pd.read_csv(accepted_file_path, usecols=["transaction_row_id"])
    return pd.DataFrame(columns=["transaction_row_id"])
...
def validate_unique_id(df_final: pd.DataFrame, output_dir: str):
    """
    ensures transaction_row_id is unique in df_final. if new duplicates are found,
    ...
    try:
        ...
        new_duplicates = duplicates[
            ~duplicates["transaction_row_id"].isin(df_accepted["transaction_row_id"])
        ]
        ...
        user_input = input("\naccept new duplicates? (y/n): ").strip().lower()
        if user_input == "y":
            logging.info("duplicates: accepted")
            df_new_accepts = new_duplicates[["transaction_row_id"]].drop_duplicates()
            df_updated_accepts = pd.concat(
                [df_accepted, df_new_accepts]
            ).drop_duplicates(subset=["transaction_row_id"])
